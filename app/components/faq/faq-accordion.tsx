...
export function FAQAccordion({ faqs, title }: FAQAccordionProps) {
  ...
  return (
    <section className="mb-12">
      {title && (
        <h2 className="text-2xl md:text-3xl font-bold mb-6" style={{ color: '#111827' }}>{title}</h2>
      )}
      <div className="space-y-4">
        {faqs.map((faq, index) => (
          ...
          >
            ...
            >
              <h3 className="text-xl font-semibold pr-4" style={{ color: '#111827' }}>{faq.question}</h3>
              <svg
            ...
            >
              <div className="px-6 pb-6">
                <p className="leading-relaxed" style={{ color: '#111827' }}>{faq.answer}</p>
              </div>
            </div>
          </div>
        ))}
